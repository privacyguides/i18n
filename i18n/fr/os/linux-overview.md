...
Les dérivés d'Arch conçus pour rendre Arch « plus facile à utiliser » ne sont pas recommandés, car ils encouragent souvent de mauvaises pratiques en matière de sécurité tout en affaiblissant le système de base. Ces pratiques consistent notamment à regrouper des paquets obsolètes dans des dépôts en aval, qui sont mis à jour moins fréquemment que les canaux officiels d'Arch. De plus, ces distributions ne parviennent généralement pas à informer correctement les utilisateurs des risques liés à l'Arch User Repository (AUR).

Toute personne utilisant [l'Arch User Repository (AUR)](https://wiki.archlinux.org/title/Arch_User_Repository) **doit** être capable de vérifier les fichiers PKGBUILD (les recettes permettant de compiler les paquets) qu'elle télécharge depuis ce service. Les paquets AUR sont des contenus créés par la communauté et ne font l'objet d'aucun contrôle ; ils sont donc exposés à [ des attaques de type «:material-package-variant-closed-remove: » visant la chaîne logistique](../basics/common-threats.md#attacks-against-certain-organizations ""){.pg-viridian} logicielle, ce qui s'est d'ailleurs déjà produit [par le passé](https://bleepingcomputer.com/news/security/malware-found-in-arch-linux-aur-package-repository) et, plus récemment, lorsque [1 500 paquets](https://discuss.privacyguides.net/t/around-1-500-aur-packages-compromised-with-rootkit-like-malware/38517) ont été compromis.
...
Si vous maîtrisez Linux et souhaitez utiliser une distribution dérivée d'Arch, la version principale d'Arch Linux est la **seule** option recommandée.
