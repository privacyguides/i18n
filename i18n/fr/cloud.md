...
**Peergos** est un protocole décentralisé et une plateforme open-source pour le stockage, les réseaux sociaux et les applications. Il fournit un espace sécurisé et privé où les utilisateurs peuvent stocker, partager, visualiser et modifier leurs photos, vidéos, documents, etc.

Peergos sécurise vos fichiers grâce au chiffrement de bout-en-bout résistant aux attaques quantiques et garantit que toutes les données relatives à vos fichiers restent privées. Il est également possible de [l'auto-héberger](https://book.peergos.org/features/self).
...
Le client, le serveur, et l'interface de ligne de commande sont tous exécutés à partir du même binaire. De plus, il est possible de [synchroniser](https://book.peergos.org/features/sync) de manière bidirectionnelle un dossier local avec un dossier Peergos (accessible via l'application de bureau et l'application Android), ainsi que d'utiliser un [pont WebDAV](https://book.peergos.org/features/webdav) pour permettre à d'autres applications d'accèder au stockage Peergos. Vous pouvez vous référer à la documentation de Peergos pour un aperçu complet de ses nombreuses fonctionnalités.
