...
Interfejsy [SafetyNet](https://developer.android.com/training/safetynet/attestation) i [Play Integrity API](https://developer.android.com/google/play/integrity) są zazwyczaj używane przez [aplikacje bankowe](https://grapheneos.org/usage#banking-apps). Wiele aplikacji bankowych działa poprawnie w systemie GrapheneOS z izolowanymi usługami Google Play, jednak niektóre aplikacje niebankowe stosują własne, prymitywne mechanizmy antymanipulacyjne, które mogą zawodzić. GrapheneOS przechodzi kontrolę `basicIntegrity`, ale nie kontrolę certyfikacji `ctsProfileMatch`. Urządzenia z Androidem 8 lub nowszym obsługują atestację sprzętową, której nie da się obejść bez wycieków kluczy lub poważnych podatności.

Jeśli chodzi o Portfel Google — nie zalecamy korzystania z tej usługi ze względu na ich [Politykę prywatności](https://payments.google.com/payments/apis-secure/get_legal_document?ldo=0&ldt=privacynotice&ldl=en), która stwierdza, że trzeba zrezygnować, jeśli nie chce się, by informacje o zdolności kredytowej i dane osobowe były udostępniane usługom marketingu afiliacyjnego.
